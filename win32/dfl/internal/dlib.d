...
{
	string stringFromStringz(in char* sz)
	{
		return std.conv.to!(string)(sz);
	}
}
...
string uintToHexString(uint num)
{
...
alias std.utf.toUTFz!(wchar*, string) utf8stringToUtf16stringz;
...
else
{
	version(DFL_DMD2029)
	{
		alias std.conv.to!(int, string) stringToInt; // D 2.029
	}
	else
	{
